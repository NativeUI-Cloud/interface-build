...
import { ShieldCheck, Search, Coins, FileCode, Bot } from 'lucide-react'; // Example icons

export const agentTemplates: AgentTemplate[] = [
  {
    id: 'anti-rug-pull-analyzer-v2', // Changed ID to differentiate if needed, or keep same and overwrite
...
Be factual and cautious. Do NOT give financial advice. Your analysis is for informational purposes only. Specify what information you could not retrieve if a tool was not available or did not provide the data.`,
  ...
  },
  {
    ...
    icon: FileCode, // Using FileCode as an icon for contract analysis
...
If a user asks about 'analyzing for risks', defer to the more specialized 'Smart Contract Rug Pull Analyzer' template if available, or perform a basic check for contract verification and ownership if explicitly asked.`,
  ...
  },
  {
    ...
    description: 'A general purpose agent that can search the web and summarize information based on user queries.',
    icon: Search,
  ...
  },
  {
    ...
    description: 'Fetches and reports the current price and market data for specified cryptocurrencies using the CoinGecko tool.',
    icon: Coins,
  ...
  },
  {
    ...
    description: 'A basic AI agent for general conversation, without specific tools pre-suggested. Can be a starting point for custom agents.',
    icon: Bot,
    systemPrompt: `You are a helpful AI assistant. Respond to the user's queries thoughtfully and clearly.`,
    suggestedToolTypes: [], // No specific tools suggested by default
  }
